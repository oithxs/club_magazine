...
# まとめ
この章では，MarkdownでHxSの部誌を生成するための環境設定について解説しました．
部誌は`Pandoc`を使ってMarkdownを中間表現としてtexファイルに変換し，そのtexファイルを`pLaTeX`でpdfに変換しました．
Markdownに対してtexlintを使うことで文章を自動校正し，人の手助けをする仕組みを導入しました．
開発環境は`Docker`を使って用意することで，環境のポータビリティを確保しました．
また，コミットのたびに部誌がCircle CI上で自動ビルドされるようにしました．